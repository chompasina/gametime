...
HealthFood.prototype.createHealthFoodsAttributesArray = function(){
  var healthFoodRate = 1500;
  var healthFoods = [];
  // var healthFoodRateOfDescent = 0.50;
  var lastHealthFood = -1;
  ...
  if(time > (lastHealthFood + healthFoodRate)){
    lastHealthFood = time;
    healthFoods.push(this.dropRandomFood());
  }
  return healthFoods;
...
HealthFood.prototype.draw = function(){
  // healthFood.y += healthFoodRateOfDescent;
  //clears previous Path that we began -- necessary to fill the circles correctly
  this.context.beginPath();
  //creates the start and end angles for our circle
  //explain how each argument of arc affects the circle
  this.context.arc(this.x, this.y,8, 0, Math.PI*2);
  // sets stage for how we'll fill circle
