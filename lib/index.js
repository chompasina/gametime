...
function recordFoodScore(foodsArray, food){
  ...
  totalPointsBar.style = "width:" + total/6 + "%";
  // totalPointsBar.innerHTML = total;
  winPointsBar.style = "width:" + (600 - total)/6 + "%";
...
function checkProgressLevel(total){
  var alertedFirst = localStorage.getItem('alertedFirst') || '';
  var alertedSecond = localStorage.getItem('alertedSecond') || '';
  var alertedWon = localStorage.getItem('alertedWon') || '';

  if(total >= 200 && total < 400 && alertedFirst !== 'yes'){
    alert("You've made it to the next level!");
    localStorage.setItem('alertedFirst','yes');
  } else if(total >= 400 && total < 600 && alertedSecond !== 'yes'){
    alert("You've made it to the final level!");
    localStorage.setItem('alertedSecond', 'yes');
  } else if(total === 600 && alertedWon !== "yes"){
    alert("You win!");
    localStorage.setItem('alertedWon', 'yes');
    window.cancelAnimationFrame(requestAnimationFrameID);
    canvas.style.display = "none";
    startButton.style.display = "block";
  } else if(alertedWon === 'yes'){
    localStorage.setItem('alertedWon', '');
    localStorage.setItem('alertedWon', '');
    localStorage.setItem('alertedWon', '');
  }
}
