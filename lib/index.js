...
function collisionOutcome(foodsArray, tray){
  for (var i = 0; i < foodsArray.length; i++){
    ...
    if (isHealthFoodCollidingWithTray(food, tray)){
      recordHealthFoodScore(foodsArray, food);
    }
  }
}

function isHealthFoodCollidingWithTray(food, tray){
  return !(
...
function recordHealthFoodScore(foodsArray, food){
