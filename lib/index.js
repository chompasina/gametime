...
function recordFoodScore(foodsArray, food){
  ...
  totalPointsBar.style = "width:" + (total/2) + "%";
  totalPointsBar.innerHTML = total;
  winPointsBar.style = "width:" + (200 - total)/2 + "%";
  winPointsBar.innerHTML = (200 - total) + " Needed to win";
  checkGameOver(total);
  checkProgressLevel(total);
}

function checkGameOver(total){
  if(total < 0){
    alert("Game over!");
...
var totalPointsPossible = document.getElementById('possible-points');
var currentPoints = document.getElementById('current-points');

function checkProgressLevel(total){
  if(total >= 200 && total < 400){
    alert("You've made it to the next level!");
    // window.cancelAnimationFrame(requestAnimationFrameID);
    totalPointsPossible.innerHTML = 400;
    currentPoints.innerHTML = 200;
    totalPointsBar.style = "width:" + (total - 200) + "%";
    totalPointsBar.innerHTML = total;
    winPointsBar.style = "width:" + (400 - total)/2 + "%";
    winPointsBar.innerHTML = (400 - total) + " Needed to win";
  } else if(total >= 400 && total < 600){
    alert("You've made it to the final level!");
    totalPointsPossible.innerHTML = 600;
    currentPoints.innerHTML = 400;
    totalPointsBar.style = "width:" + (total - 600) + "%";
    totalPointsBar.innerHTML = total;
    winPointsBar.style = "width:" + (600 - total)/2 + "%";
    winPointsBar.innerHTML = (600 - total) + " Needed to win";
  } else if(total === 600)
}
