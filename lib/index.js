...
var healthFoods = [];
var starterJunkFood = new JunkFood();
var junkFoods = [starterJunkFood];

function drawJunkFood(junkFoods){
...
function drawHealthFood(healthFoods){
...
requestAnimationFrame(function gameLoop(overlord, junkFoods, healthFoods){
  world.draw();
  drawOverlord();
  oscillate(overlord, world);
  drawJunkFood(junkFoods);
  drawHealthFood(healthFoods);

  requestAnimationFrame(gameLoop);
});

window.setInterval(createHealthFood, 500, context);
window.setInterval(createJunkFood, 500, context, overlord);
...
function oscillate(overlord, world){
  ...
  // createJunkFood(context, overlord.x);

  // return this;
}

function createJunkFood(context, object){
    var randomJunkFoodAttributes = new RandomJunkFood(object.x);
...
function junkFoodDraw(junkFood){
  context.beginPath();
  context.arc(overlord.x, junkFood.y, 8, 0, Math.PI*2);
